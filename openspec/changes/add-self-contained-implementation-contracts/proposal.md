...
- Make `gdc check --verify-impl` and `gdc diff` honor schema 1.1 module type contracts as the module's concrete implementation symbols and select exact C# generic types and overloads from multi-type source files.
- Resolve implementation packet targets by exact YAML stem or unambiguous canonical, bare, and kebab-case node IDs.
